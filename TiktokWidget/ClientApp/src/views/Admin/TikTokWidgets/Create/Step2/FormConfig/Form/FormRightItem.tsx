...
function FormRightItem() {
  ...
  const onSetNumberItems = (val: string = "") => {
    if (val === "0") return;
    const numberItems = parseInt(val);
    return dispatch(WidgetActionTS.OnSetSetting({ numberItems: numberItems }));
  };

  const onBlurNumberItems = () => {
    let val = widgetReducer.settings.numberItems;
    if (!val) val = 8;
    return dispatch(WidgetActionTS.OnSetSetting({ numberItems: val }));
  };
  ...
  return (
    <FormRight>
      <FormRightWrapper>
        ...
        <FormControlStep style={{ alignItems: "flex-start" }}>
          <LabelForm>
            <TextStyle>Number items</TextStyle>
          </LabelForm>
          <InputForm>
            <TextField
              name="number-items"
              ...
              label=""
              type="number"
              value={String(widgetReducer.settings.numberItems)}
              onChange={onSetNumberItems}
              onBlur={onBlurNumberItems}
