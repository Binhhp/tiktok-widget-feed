...
const TiktokVideoPreview = React.memo(function TiktokVideoPreview(
  ...
  const fetchData = async () => {
    ...
    const valueSource =
      props.optionsShowItem?.valueSource ?? tiktokReducer.settings.valueSource;

    let sourceType = tiktokReducer.settings.source;
    if (props.optionsShowItem?.sourceType) {
      //Get value from Enum Source in Widget Manager
      const indexOfS = Object.keys(SourceTypeEnum).indexOf(
        props.optionsShowItem.sourceType
      );
      sourceType = Object.values(SourceTypeEnum)[indexOfS] as number;
    }
  ...
  return (
    <Modal
      ...
      open={props.active}
      title={<ImportVideoTitle>Import the Tiktok Video</ImportVideoTitle>}
      ...
      <Modal.Section>
        <VideoPreviewContainer>
          <div className="header">
            <div>
              "{state.data.length}" videos found from{" "}
              <strong>
                @
